...
def ftp():
    ...
    authorizer.add_user('admin','hi123','.',perm='elradfmw')
...
def handleMessageError(client):
    message = '''
        You need to select any user before sending any message.
        Click on REFRESH to see all available users. 
    '''
    client.send(message.encode())


def sendText(client_name,message):
    global clients

    otherClientName = clients[client_name]["connected_with"]
    otherClientSocket = clients[otherClientName]["client"]

    finalMessage = client_name + '>' + message
    otherClientSocket.send(finalMessage.encode())
...
def handleMessges(client, message, client_name):
    if(message == 'show list'):
        ...
        if(connected):
            sendText(client_name, message)
        else:
            handleMessageError(client)
...
#MULTIPLE REQUESTS FOR FILE TRANSFERS
